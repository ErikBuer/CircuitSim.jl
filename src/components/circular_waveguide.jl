"""
    CircularWaveguide <: AbstractTransmissionLine

2-port circular waveguide in TE11 mode (CIRCLINE).
...
- `n1::Int`: Port 1 node
- `n2::Int`: Port 2 node
- `a::Float64`: Waveguide radius in meters (default: 2.86e-2, must be > 0)
...
```jldoctest
julia> wg = CircularWaveguide("CW1")
CircularWaveguide("CW1", 0, 0, 0.0286, 1.5, 1.0, 1.0, 0.0004, 2.2e-8, 26.85, "unspecified")
```
"""
mutable struct CircularWaveguide <: AbstractTransmissionLine
    name::String
    ...
    function CircularWaveguide(name::AbstractString;
        a::Real=2.86e-2,
        ...
        new(String(name), 0, 0,
            Float64(a), Float64(length_m),
            Float64(er), Float64(mur), Float64(tand), Float64(rho),
            Float64(temp), material)
    end
end

function to_qucs_netlist(comp::CircularWaveguide)::String
    params = "a=\"$(comp.a)\" L=\"$(comp.length_m)\"" *
             " er=\"$(comp.er)\" mur=\"$(comp.mur)\"" *
             " tand=\"$(comp.tand)\" rho=\"$(comp.rho)\"" *
             " Temp=\"$(comp.temp)\" Material=\"$(comp.material)\""
    return "CIRCLINE:$(comp.name) $(qucs_node(comp.n1)) $(qucs_node(comp.n2)) $params"
end

function _get_node_number(comp::CircularWaveguide, pin::Symbol)
    if pin == :n1
        return comp.n1
    elseif pin == :n2
        return comp.n2
    else
        error("Invalid pin $pin for CircularWaveguide. Use :n1 or :n2")
    end
end

function _set_node_number!(comp::CircularWaveguide, pin::Symbol, node::Int)
    if pin == :n1
        comp.n1 = node
    elseif pin == :n2
        comp.n2 = node
...
function get_pins(::CircularWaveguide)
    return [:n1, :n2]
end
