...
mutable struct ACVoltageSource <: AbstractACVoltageSource
    ...
    ACVoltageSource(name::AbstractString;
        ac_magnitude::Real,
        dc_voltage::Real=0.0,
        ac_phase::Real=0.0,
        freq::Real=1e6
    ) = new(String(name), 0, 0, dc_voltage, ac_magnitude, ac_phase, freq)
end

function to_qucs_netlist(comp::ACVoltageSource)::String
    "Vac:$(comp.name) $(qucs_node(comp.nplus)) $(qucs_node(comp.nminus)) U=\"$(format_value(comp.ac_magnitude))\" f=\"$(format_value(comp.freq))\" Phase=\"$(comp.ac_phase)\""
end

function to_spice_netlist(comp::ACVoltageSource)::String
    if comp.dc_voltage != 0.0
        "V$(comp.name) $(comp.nplus) $(comp.nminus) DC $(comp.dc_voltage) AC $(comp.ac_magnitude) $(comp.ac_phase) SIN($(comp.dc_voltage) $(comp.ac_magnitude) $(comp.freq) 0 0 $(comp.ac_phase))"
    else
        "V$(comp.name) $(comp.nplus) $(comp.nminus) AC $(comp.ac_magnitude) $(comp.ac_phase) SIN(0 $(comp.ac_magnitude) $(comp.freq) 0 0 $(comp.ac_phase))"
    end
end
